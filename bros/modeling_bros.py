...
class BrosEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if getattr(config, "max_2d_position_embeddings", False):
            self.line_rank_embeddings = nn.Embedding(
                config.max_position_embeddings, config.hidden_size)

            self.line_rank_inner_embeddings = nn.Embedding(
    ...
    def forward(
        ...
        if self.pe_type == "pdpdq_ws":
            ...
            bbox_pos_scores_1 = torch.einsum(
                "bnid,bnjd->bnij", (torch.mul(query_layer, head_q_pos), head_k_pos)
            )
            bbox_pos_scores_2 = torch.einsum(
                "bnid,bnjd->bnij", (head_q_pos, head_k_pos)
            )
            ...
            bbox_pos_scores = torch.einsum(
                "bnid,bijd->bnij", (query_layer, bbox_pos_emb)
            )
        else:
...
class BrosEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if getattr(self.config, "gradient_checkpoint", False) and self.training:

                if use_cache:
                    logger.warning(
                        "`use_cache=True` is incompatible with `config.gradient_checkpoint=True`. Setting "
                        "`use_cache=False`..."
                    )
                    use_cache = False
...
class BrosForPreTraining(BrosPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=BrosForPreTrainingOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        input_ids=None,
...
class BrosLMHeadModel(BrosPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `BrosLMHeadModel` as a standalone, add `is_decoder=True.`"
            )
...
@add_start_docstrings(
    """Bert Model with a `language modeling` head on top. """, BROS_START_DOCSTRING
)
class BrosForMaskedLM(BrosPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        print(attention_mask.new_zeros((attention_mask.shape[0], 1)).type)
        ...
        input_ids = torch.cat([input_ids.long(), dummy_token], dim=1)
