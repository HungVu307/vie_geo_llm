...
def get_callbacks(cfg):
    ...
    cb1 = CustomModelCheckpoint(
        dirpath=cfg.save_weight_dir, filename='HANAM-{epoch}-{f1_labeling:.4f}', monitor="f1_labeling", save_top_k=1, mode='max',
    ...
    cb2 = CustomModelCheckpoint(
        dirpath=cfg.save_weight_dir, filename='HANAM-{epoch}-{f1_linking:.4f}', monitor="f1_linking", save_top_k=1, mode='max',
         save_last=False, every_n_epochs=1, save_on_train_epoch_end=False
    )
